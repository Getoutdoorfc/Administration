...
class MsTokenHandler {
    ...
    public function store_tokens(array $tokens): bool {
        if (!isset($tokens['access_token'], $tokens['refresh_token'], $tokens['expires_in'])) {
            LoggerManager::getInstance()->error('Invalid token data provided for storage.');
            return false;
        }
        ...
        try {
            ...
            LoggerManager::getInstance()->info('Tokens stored successfully.', [
                'expires_in' => $tokens['expires_in'],
            ]);
            return true;
        } catch (\Exception $e) {
            LoggerManager::getInstance()->error('Failed to store tokens: ' . $e->getMessage());
            return false;
        }
    }
    ...
    public function get_access_token() {
        LoggerManager::getInstance()->info('Fetching access token...');
        ...
        if ($this->is_token_expired($expires)) {
            LoggerManager::getInstance()->info('Access token expired. Attempting to refresh...');
            if (!$this->refresh_access_token()) {
                LoggerManager::getInstance()->error('Failed to refresh access token.');
                return false;
            }
            $access_token = get_option('administration_microsoft_access_token', '');
        }
        ...
        if (empty($access_token)) {
            LoggerManager::getInstance()->error('Access token is not available.');
            return false;
        }
    ...
    private function refresh_access_token(): bool {
        ...
        if (empty($refresh_token)) {
            LoggerManager::getInstance()->error('Refresh token not available.');
            return false;
        }
        ...
        $token_endpoint = 'https://login.microsoftonline.com/' . WpConfigHandler::get_config('microsoft_tenant_id') . '/oauth2/v2.0/token';

        LoggerManager::getInstance()->info('Sending request to refresh access token.');
        ...
        if (is_wp_error($response)) {
            ...
            LoggerManager::getInstance()->error('Token refresh failed: ' . $error_message);
            return false;
        }
        ...
        if (isset($body['access_token'], $body['refresh_token'])) {
            LoggerManager::getInstance()->info('Tokens refreshed successfully.', [
                'expires_in' => $body['expires_in'] ?? 'unknown',
            ]);
            return $this->store_tokens($body);
        } else {
            LoggerManager::getInstance()->error('Invalid response during token refresh.', [
                'response_body' => $body,
            ]);
            return false;
        }
    ...
    private function is_token_expired(int $expires): bool {
        ...
        LoggerManager::getInstance()->info('Checking if token is expired.', [
            'expires_at' => $expires,
            'current_time' => time(),
