<?php
namespace Administration\Modules\WordPress\WordPressPages;
...
class MicrosoftSetupPage {
    ...
    public function render_setup_page() {
        ...
        loggerManager::getInstance()->info("Rendering Microsoft Setup Page.");
        ...
        loggerManager::getInstance()->info("Microsoft Setup Page rendered successfully.");
    }
    ...
    private function render_status_message($credentials_saved) {
        if ($credentials_saved) {
            WordPressNotices::display_message(__('Credentials are saved. You can now log in with Microsoft.', 'administration'), 'updated');
        } else {
            WordPressNotices::display_message(__('Please enter your credentials and save them to enable login.', 'administration'), 'warning');
        }
    }
    ...
    private function handle_form_submission() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (!isset($_POST['security']) || !wp_verify_nonce($_POST['security'], 'administration_microsoft_options_verify')) {
                WordPressNotices::add_dynamic_notice(__('Invalid nonce. Please try again.', 'administration'), 'error');
                return;
            }
            ...
            if (empty($client_id) || empty($client_secret) || empty($tenant_id)) {
                WordPressNotices::add_dynamic_notice(__('All fields are required.', 'administration'), 'error');
                return;
            }
            ...
            if (
                ...
                WordPressNotices::add_dynamic_notice(__('Settings saved successfully.', 'administration'), 'updated');
            } else {
                WordPressNotices::add_dynamic_notice(__('Failed to save settings. Check logs for details.', 'administration'), 'error');
            }
        }
    }
